...
class TestList(unittest.TestCase):
    def test_yieldd(self):
        linked_list = Pair(1, Pair(2, Pair(3, None)))
        for value in yield_iterator(linked_list):
            print(value)
    def test_pairr(self):
        linked_list = Pair(1, None)
        self.assertEqual(linked_list.rest, None)
    def test_reprrr(self):
        self.assertEqual(repr(Iterator(Pair(1, Pair(2, Pair(3, None))))), "LinkedList: Pair(1, Pair(2, Pair(3, None)))")
        self.assertEqual(repr(Pair(1, None)), "Pair(1, None)")
    def test_object_iterator(self):
        linked_list = Pair(1, Pair(2, Pair(3, None)))
        object1 = object_iterator(linked_list)
        self.assertEqual(object1, Iterator(linked_list))
    def test_has_next(self):
        linked_list = Pair(1, Pair(2, Pair(3, None)))
        iterator_object = Iterator(linked_list)
        self.assertTrue(has_next(iterator_object))
        iterator_object2 = Iterator(None)
        self.assertFalse(has_next(iterator_object2))
        self.assertTrue(has_next(Iterator(Pair(1, None))))
        iterator_object = Iterator(Pair(1, Pair(2, None)))
        self.assertTrue(has_next(iterator_object))
        self.assertFalse(has_next(Pair(None, None)))
    def test_next(self):
        iterator_object = Iterator(None)
        with self.assertRaises(StopIteration):
            next(iterator_object)
        iterator_object2 = Iterator(Pair(1, Pair(2, Pair(3, None))))
        self.assertEqual(next(iterator_object2), 1)
        i = Iterator(Pair(-5, Pair(0, Pair(-10, None))))
        print(next(i))
        print(next(i))
        print(next(i))
        with self.assertRaises(StopIteration):
