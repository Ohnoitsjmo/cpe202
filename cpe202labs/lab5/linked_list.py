...
# an Iterator is a wrapper class that tracks the position of the iterator within a linked list
class Iterator:
    def __init__(self, linked_list):
        self.linked_list = linked_list

    def __repr__(self):
        return "LinkedList: {!r}".format(self.linked_list)
...
# LinkedList -> value
# Takes in a LinkedList and yields each value within the list
def yield_iterator(linked_list):
    yield linked_list.first
    while linked_list.rest is not None:
        yield linked_list.rest.first
        linked_list = linked_list.rest

# LinkedList -> Object
# Takes in a linked list and returns an iterator object
def object_iterator(linked_list):
    return Iterator(linked_list)

# Object -> bool
# Takes in an iterator object and returns a bool based on whether there's another object
def has_next(iterator_object):
    return iterator_object.linked_list != None

# Object -> value
# Takes in an iterator object and returns the next value. Raises a StopIteration exception if there is no next value
def next(iterator_object):
    if iterator_object.linked_list == None:
        raise StopIteration 
    else:
        first = iterator_object.linked_list.first
        iterator_object.linked_list = iterator_object.linked_list.rest
        return first
